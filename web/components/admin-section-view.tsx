...
const copy: Record<Section, [string, string]> = {
  dashboard: ["Dashboard", "Live system health, usage, and activity."], users: ["Users", "Manage persisted identities and account state."], grants: ["Grants", "Control persisted resource scopes and expirations."], tokens: ["Tokens", "Review and revoke issued API credentials."], providers: ["Providers", "Install provider manifests exposed by this instance."], ingestion: ["Ingestion jobs", "Monitor real provider materialization jobs."], storage: ["Storage", "Review artifact storage measured from the catalog."], monitoring: ["Monitoring", "Inspect recorded API traffic and backend health."], audit: ["Audit log", "Review persisted governance events."], security: ["Security", "Configure enforced authentication policies."], backups: ["Backups", "Create and review metadata snapshots in object storage."], settings: ["System Settings", "Persist instance configuration in PostgreSQL."],
};
