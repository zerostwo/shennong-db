...
function DataTable({ rows, section }: { rows: unknown[]; section: Section }) {
  ...
  return objects.length ? <table className="simple-table"><thead><tr><th>{section === "providers" ? "Provider" : section === "audit" ? "Action" : "User"}</th><th>Details</th></tr></thead><tbody>{objects.map((row, index) => <tr key={String(row.id ?? row.event_id ?? index)}><td>{String(row.name ?? row.action ?? row.display_name ?? row.id ?? row.event_id ?? "—")}</td><td><code>{JSON.stringify(row)}</code></td></tr>)}</tbody></table> : <div className="empty-state"><FileText /><h3>No records returned</h3><p>The API returned an empty collection.</p></div>;
}
