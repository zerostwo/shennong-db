...
vi.mock("@/lib/api/adapter", () => ({ getSession: async () => ({ authenticated:false, user_id:"", role:"", scopes:[] }), getPublicConfig:async()=>({registration_mode:"disabled"}), getHealth:async()=>({status:"ok"}), listIngestionJobs:async()=>[], signOut: async () => undefined, uploadFile:async()=>({id:"upload-1"}),registerUploads:async()=>({id:"resource-1"}),registerProjectUploads:async()=>({id:"resource-1"}) }));
