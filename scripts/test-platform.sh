...
curl --noproxy '*' --fail --silent -H "$json" \
  -d '{"resource":"fixture-public","operation":"expression","features":[{"type":"gene","name":"GENE1"}],"options":{"limit":2,"format":"jsonl"}}' \
  "$base/api/v1/query/stream" > "$tmpdir/query.jsonl"
[ "$(wc -l < "$tmpdir/query.jsonl" | tr -d ' ')" = 2 ]
curl --noproxy '*' --fail --silent -H "$json" \
